...
def aba():
    ...
    nscrollbar = tk.Scrollbar(aba2, command=ntexto.yview).pack(side=RIGHT, expand=True, fill='both')
    ntexto.pack()
    nl.pack()
    np.pack()
    nblimpar.pack()
...
scrollbar = tk.Scrollbar(aba1, command=texto.yview).pack(side=RIGHT, expand=True, fill='both')
